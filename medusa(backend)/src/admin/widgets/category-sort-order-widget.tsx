  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        console.error("Failed to load category sort order", e)
      } finally {
        setLoading(false)
      }
    }
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      ...
      try {
        ...
        setSaveError(true)
        console.error("Failed to save category sort order", e)
      } finally {
        setSaving(false)
      }
    }, 600)
  ...
  return (
    <Container className="divide-y p-0">
      <div className="flex items-center justify-between px-6 py-4">
        <Heading level="h2">Category Sort Order</Heading>
      ...
      <div className="flex flex-col gap-2 px-6 py-4">
        ...
        {validationError ? (
          ...
          <Text className="text-sm text-ui-fg-subtle">
            Lower number appears first. Leave empty to use name-based fallback.
          </Text>
        )}
      </div>
    </Container>
...
export default CategorySortOrderWidget
