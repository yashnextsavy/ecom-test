...
const CategoryTrendingOffersWidget = () => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        setChecked(toBoolean(metadata.show_in_trending_offers))
      } catch (e) {
        console.error("Failed to load category trending offers visibility", e)
      } finally {
        setLoading(false)
      }
    }
    ...
    try {
      ...
      const saveRes = await fetch(`/admin/product-categories/${id}`, {
        ...
        body: JSON.stringify({
          metadata: {
            ...metadata,
            show_in_trending_offers: nextChecked,
          },
        }),
      })
      ...
      setSaveError(true)
      setChecked(!nextChecked)
      console.error("Failed to save category trending offers visibility", e)
    } finally {
      setSaving(false)
    }
  }
  ...
  return (
    <Container className="divide-y p-0">
      <div className="flex items-center justify-between px-6 py-4">
        <Heading level="h2">Trending Offers</Heading>
      ...
      <div className="flex items-center gap-2 px-6 py-4">
        ...
        />
        <Text className="text-sm font-medium">Show in trending offers</Text>
      </div>
    </Container>
  )
}
...
export default CategoryTrendingOffersWidget
