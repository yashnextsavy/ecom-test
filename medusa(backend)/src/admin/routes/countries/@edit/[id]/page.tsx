...
const EditCountry = () => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      const { country } = await sdk.client.fetch<{ country: Country }>(
        `/admin/international-countries/${id}`
      )
    ...
    load().catch(() => {
      // no-op: drawer can be closed by user.
    })
  ...
  const handleSave = async () => {
    ...
    setLoading(true)
    try {
      await sdk.client.fetch(`/admin/international-countries/${id}`, {
        method: "post",
        body: {
  ...
  return (
    <Drawer open={open} onOpenChange={setOpen}>
      <Drawer.Content>
        <Drawer.Header>
          <Drawer.Title>Edit Country</Drawer.Title>
        </Drawer.Header>
