...
const enrichCategories = (
  ...
  return categories.map((category) => {
    ...
    const seoMeta: SeoMeta = {
      title: toText(metadata.seo_meta_title),
      description: toText(metadata.seo_meta_description),
    }
    ...
    return {
      ...
      metadata: {
        ...
        seo_meta_title: seoMeta.title,
        seo_meta_description: seoMeta.description,
      ...
      additional_information: additionalInformation,
      seo: [seoMeta.title, seoMeta.description],
      seo_meta: seoMeta,
      faq_section: faqSection,
      faq,
      listing_page_banner: listingPageBanner,
...
export async function GET(req: MedusaRequest, res: MedusaResponse): Promise<void> {
  ...
  const filteredProducts = (requestedHandles.length
    ? productRows.filter((product) =>
        (product.categories || []).some((category) => selectedCategoryIds.has(category.id))
      )
    : productRows
  ).filter((product) => {
    if (requestedHandles.length && toText(product.status).trim().toLowerCase() === "draft") {
      return false
    }
  ...
  const attachedExamSeriesIds = Array.from(
    new Set(
      filteredProducts.flatMap((product) => getExamSeriesIdsFromMetadata(product.metadata))
    )
  )
  ...
  if (!requestedHandles.length) {
    for (const product of filteredProducts) {
      for (const category of product.categories || []) {
        ...
        if (!relevantCategoryMap.has(category.id)) {
          relevantCategoryMap.set(category.id, category)
        }
      }
    }
  ...
  res.status(200).json({
    ...
    counts: {
      categories: enrichedParents.length,
      products: sortedProducts.length,
    },
    categories: enrichedParents.map((category) => ({
      ...category,
      fallbackDescription: toText(
        (category.metadata as Record<string, unknown> | null | undefined)?.fallbackDescription
      ),
