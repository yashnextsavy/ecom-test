...
export async function GET(
  ...
  const trendingOfferCategories = sortCategories(
    (categories as CategoryRecord[]).filter((category) =>
      toBoolean(category.metadata?.show_in_trending_offers)
    )
  )

  const categoryIds = trendingOfferCategories.map((category) => category.id)
  ...
  const productsByCategoryId = productRows.reduce((acc, product) => {
  ...
  const responseCategories = trendingOfferCategories.map((category) => {
    const metadata = (category.metadata || {}) as Record<string, unknown>
    const showInTrendingOffers = toBoolean(metadata.show_in_trending_offers)
    ...
    return {
      ...
      show_in_trending_offers: showInTrendingOffers,
