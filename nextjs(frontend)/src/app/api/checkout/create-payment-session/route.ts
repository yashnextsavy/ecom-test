...
function normalizeEasebuzzPayload(data: any, selectedProviderId: string) {
  ...
  const paymentMethodRaw = session?.payment_method || session?.data?.payment_method || "POST"
  const paymentMethod =
...
export async function POST(req: Request) {
  try {
    const { payment_collection_id, cart_id, email, phone, first_name } = await req.json()

    if (!payment_collection_id) {
      return NextResponse.json({ error: "payment_collection_id is required" }, { status: 400 })
    }
